...
from data_processing.data_extraction import extract_in_braces, load_config
...
def plot_dict_gridspec(all_data, title, num_classes=2):
  ...
  ax3 = f.add_subplot(gs[1,2], sharex=ax2)
  ...
  for ind, key in enumerate(all_data.keys()):
    ...
    ax[1].plot(all_data[key]['train_acc'], label=label, ls='dotted', color=colors[ind])
    ax[1].plot(all_data[key]['test_acc'],  color=colors[ind])
    ax[2].scatter(label, 1. - all_data[key]['test_acc'][-1], color=colors[ind])
    ax[3].scatter(label, all_data[key]['test_loss'][-1], marker='+', color=colors[ind])


  ax[0].set_ylim(0, np.log(num_classes) + 0.1)
  ax[1].set_ylim(0, 1)
  ax[0].set_ylabel('Cross-entropy loss')
  ax[1].set_ylabel('Accuracy')
  ax[2].set_xlabel('Learning Rate')
  ...
  ax[3].set_ylabel('Test Loss')
  ax[2].set_ylim(1./num_classes - 0.1, 1. + 0.1)
  ax[2].loglog()
  ax[2].set_ylim(0.06, 1)
  ax[3].loglog()
  f.suptitle(title, fontsize=24)
...
def extract_optimum_test_acc(full_datadict, attribute):
...
def group_datadict_by_attributes(full_datadict, attributes, config=None):
  ...
  modification_func = lambda k: tuple([k[i] for i in other_indices])
  modified_dicts = {k: modify_keys(v,modification_func) for k,v in grouped_dict.items()}

  return modified_dicts

def subset_datadict_by_attributes(full_datadict, attributes, values, config=None):
  """ full_datadict has keys which are tuples, and we want to group the dictionary by attributes """

  grouped_dict = group_datadict_by_attributes(full_datadict, attributes, config)
